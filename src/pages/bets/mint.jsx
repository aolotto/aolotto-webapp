    ...
    <section className="response_cols overflow-visible border-y border-current/20 py-6 px-2">
        ...
        <div className="w-full h-full flex flex-col md:flex-row gap-4">
            <div className="md:w-[36%] h-full hidden lg:flex flex-col justify-between items-center md:items-start gap-4">
              ...
              <div className="text-xs flex flex-col gap-2">
                <p><Icon icon="ph:arrow-elbow-down-right-light"/> <span className="text-current/50">Balance: </span> <span><Show when={pool?.state=="ready"} fallback={<span className="skeleton h-[1em] inline-block w-[4em]"></span>}>{toBalanceValue(pool()?.minting?.quota?.[0],12)} $ALT</Show></span></p>
                <p><Icon icon="ph:arrow-elbow-down-right-light"/> <span className="text-current/50">Quota: </span> <span><Show when={pool?.state=="ready"} fallback={<span className="skeleton h-[1em] inline-block w-[4em]"></span>}>{toBalanceValue(pool()?.minting?.quota?.[1],12)} $ALT</Show></span></p>
              </div>
            </div>
      ...
      <div className="col-span-full md:col-span-4 lg:col-start-9 ">
        ...
        <li class="text-xs md:text-sm text-current/50 flex items-center gap-2">
          ...
          <span>
            bet <span class="text-base-content">$100</span> → <span class="text-base-content">~<Show when={pool.state =="ready"} fallback={<div class="skeleton h-[1em] w-[4em] inline-block"></div>}>{toBalanceValue(minting()?.per_reward * 100 * 1,12)}</Show></span> $ALT
          </span>
        </li>
        <li class="text-xs md:text-sm text-current/50 flex items-center gap-2">
          <span className="tooltip tooltip-bottom" data-tip="1 $ALT = 1 $ALT">
          ...
          <span>
            bet <span class="text-base-content">$50-99</span> → <span class="text-base-content">~<Show when={pool.state =="ready"} fallback={<div class="skeleton h-[1em] w-[4em] inline-block"></div>}>{toBalanceValue(minting()?.per_reward * 50 * 0.6,12)}-{toBalanceValue(minting()?.per_reward * 99 * 0.6,12)}</Show></span> $ALT
          </span>
        </li>
        <li class="text-xs md:text-sm text-current/50 flex items-center gap-2">
          <span className="tooltip tooltip-bottom" data-tip="1 $ALT = 1 $ALT">
          ...
          <span>
            bet <span class="text-base-content">$10-49</span> → <span class="text-base-content">~<Show when={pool.state =="ready"} fallback={<div class="skeleton h-[1em] w-[4em] inline-block"></div>}>{toBalanceValue(minting()?.per_reward * 10 * 0.3,12)}-{toBalanceValue(minting()?.per_reward * 49 * 0.3,12)}</Show></span> $ALT
          </span>
        </li>
        <li class="text-xs md:text-sm text-current/50 flex items-center gap-2">
          <span className="tooltip tooltip-bottom" data-tip="1 $ALT = 1 $ALT">
          ...
          <span>
            bet <span class="text-base-content">$1-9</span> → <span class="text-base-content">~<Show when={pool.state =="ready"} fallback={<div class="skeleton h-[1em] w-[4em] inline-block"></div>}>{toBalanceValue(minting()?.per_reward * 1 * 0.1,12)}-{toBalanceValue(minting()?.per_reward * 9 * 0.1,12)}</Show></span> $ALT
          </span>
        </li>
        </ul>
      </div>
